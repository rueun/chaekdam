...
describe('Highlight', () => {
  describe('fromText', () => {
    it('텍스트 구절로부터 TEXT 출처의 한 줄을 만든다', () => {
      const highlight = Highlight.fromText('owner', 'book-1', '인상 깊은 한 구절');
    ...
    it('본문 앞뒤 공백을 제거한다', () => {
      const highlight = Highlight.fromText('owner', 'book-1', '  여백이 있는 문장  ');
      expect(highlight.content).toBe('여백이 있는 문장');
    });

    it('빈 본문으로는 만들 수 없다', () => {
      expect(() => Highlight.fromText('owner', 'book-1', '')).toThrow(EmptyHighlightContentError);
    });

    it('공백뿐인 본문으로는 만들 수 없다', () => {
      expect(() => Highlight.fromText('owner', 'book-1', '   ')).toThrow(
        EmptyHighlightContentError,
      );
    });
    ...
    it('최대 길이를 초과하면 만들 수 없다', () => {
      ...
      expect(() => Highlight.fromText('owner', 'book-1', tooLong)).toThrow(
        HighlightContentTooLongError,
      );
    });
    ...
    it('최대 길이와 같으면 만들 수 있다', () => {
      ...
      expect(() => Highlight.fromText('owner', 'book-1', exact)).not.toThrow();
    });

    it('페이지를 함께 기록할 수 있다', () => {
      const highlight = Highlight.fromText('owner', 'book-1', '문장', 'p.42');
  ...
  describe('fromPhoto', () => {
    it('사진에서 추출한 구절로 PHOTO 출처의 한 줄을 만든다', () => {
      const highlight = Highlight.fromPhoto('owner', 'book-1', 'https://x/y.jpg', '추출된 문장');
    ...
    it('사진 URL이 없으면 만들 수 없다', () => {
      expect(() => Highlight.fromPhoto('owner', 'book-1', '', '문장')).toThrow(
        MissingPhotoUrlError,
      );
    });
    ...
    it('사진 URL 앞뒤 공백을 제거해 저장한다', () => {
      const highlight = Highlight.fromPhoto('owner', 'book-1', '  https://x/y.jpg  ', '문장');
      expect(highlight.photoUrl).toBe('https://x/y.jpg');
    });

    it('추출 본문이 비어 있으면 만들 수 없다', () => {
      expect(() => Highlight.fromPhoto('owner', 'book-1', 'https://x/y.jpg', '  ')).toThrow(
        EmptyHighlightContentError,
      );
    });
  });
  ...
  describe('불변성', () => {
    it('생성된 한 줄은 동결되어 필드를 바꿀 수 없다', () => {
      const highlight = Highlight.fromText('owner', 'book-1', '문장');
      expect(Object.isFrozen(highlight)).toBe(true);
      expect(() => {
    ...
    it('생성된 한 줄마다 고유 id를 가진다', () => {
      const a = Highlight.fromText('owner', 'book-1', '문장');
      const b = Highlight.fromText('owner', 'book-1', '문장');
  ...
  describe('edit', () => {
    it('본문·페이지를 바꾼 새 한 줄을 반환하고 원본은 그대로다', () => {
      const original = Highlight.fromText('owner', 'book-1', '원래 문장', '10');
    ...
    it('page 를 생략하면 기존 페이지를 유지한다', () => {
      const original = Highlight.fromText('owner', 'book-1', '문장', '10');
    ...
    it('빈 본문으로는 수정할 수 없다', () => {
      const original = Highlight.fromText('owner', 'book-1', '문장');
  ...
  describe('moveTo', () => {
    it('다른 책으로 옮긴 새 한 줄을 반환하고 원본은 그대로다', () => {
      const original = Highlight.fromText('owner', 'book-a', '문장');
  ...
  describe('pin · archive', () => {
    ...
    it('pin/unpin 은 원본을 보존한 새 한 줄을 반환한다', () => {
      const original = Highlight.fromText('owner', 'b1', '문장');
    ...
    it('archive 는 보관하면서 고정을 함께 해제한다(불변식)', () => {
      const pinned = Highlight.fromText('owner', 'b1', '문장').pin();
    ...
    it('unarchive 는 보관을 해제한다', () => {
      const archived = Highlight.fromText('owner', 'b1', '문장').archive();
  ...
  describe('tags', () => {
    it('기본 태그는 빈 배열이다', () => {
      expect(Highlight.fromText('owner', 'b1', '문장').tags).toEqual([]);
    });

    it('태그를 정규화한다 — 공백 제거·빈 제거·대소문자 무시 중복 제거', () => {
      const h = Highlight.fromText('owner', 'b1', '문장', null, [' 위로 ', '위로', '', '성장']);
      expect(h.tags).toEqual(['위로', '성장']);
    });

    it('태그 개수를 상한(10)으로 제한한다', () => {
      const many = Array.from({ length: 15 }, (_, i) => `태그${i}`);
      expect(Highlight.fromText('owner', 'b1', '문장', null, many).tags).toHaveLength(10);
    });

    it('30자를 넘는 태그는 버린다', () => {
      const tooLong = 'ㄱ'.repeat(31);
      expect(Highlight.fromText('owner', 'b1', '문장', null, [tooLong, '짧은']).tags).toEqual([
        '짧은',
      ]);
    });

    it('모두 공백/빈 문자열이면 빈 배열이다', () => {
      expect(Highlight.fromText('owner', 'b1', '문장', null, ['', '   ']).tags).toEqual([]);
    });

    it('edit 으로 태그를 교체한다(undefined 면 유지)', () => {
      const h = Highlight.fromText('owner', 'b1', '문장', null, ['위로']);
