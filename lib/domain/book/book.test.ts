...
describe('Book', () => {
  describe('register', () => {
    ...
    it('제목 앞뒤 공백을 제거한다', () => {
      expect(Book.register({ title: '  데미안  ' }).title).toBe('데미안');
    });

    it('빈 제목으로는 담을 수 없다', () => {
      expect(() => Book.register({ title: '   ' })).toThrow(EmptyBookTitleError);
    });

    it('최대 길이를 초과하면 담을 수 없다', () => {
      const tooLong = 'ㄱ'.repeat(BOOK_TITLE_MAX_LENGTH + 1);
      expect(() => Book.register({ title: tooLong })).toThrow(BookTitleTooLongError);
    });

    it('최대 길이와 같으면 담을 수 있다', () => {
      const exact = 'ㄱ'.repeat(BOOK_TITLE_MAX_LENGTH);
      expect(() => Book.register({ title: exact })).not.toThrow();
    });

    it('저자 미지정 시 빈 문자열', () => {
      expect(Book.register({ title: '데미안' }).author).toBe('');
    });

    it('책마다 고유 id를 가진다', () => {
      expect(Book.register({ title: '데미안' }).id).not.toBe(Book.register({ title: '데미안' }).id);
    });
  });
  ...
  describe('withStatus', () => {
    ...
    it('같은 상태로 바꾸면 동일 인스턴스를 반환한다', () => {
      const reading = Book.register({ title: '데미안', status: BookStatus.READING });
